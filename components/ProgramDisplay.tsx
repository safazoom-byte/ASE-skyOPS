  ...
  if (!sortedPrograms.length) return (
    <div className="py-40 text-center bg-white rounded-[4rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center space-y-8">
      ...
      <div>
        ...
        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 max-w-sm mx-auto leading-relaxed">
          Initialize the build sequence to view the station program.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-700 pb-32">
      <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-sm flex flex-col xl:flex-row justify-between items-center gap-10">
        <div>
          <h2 className="text-3xl font-black text-slate-900 uppercase italic tracking-tighter leading-none mb-3">Station Weekly Program</h2>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{formattedStartDate} — {formattedEndDate}</p>
        </div>
        <button onClick={exportPDF} className="px-8 py-5 bg-slate-950 text-white rounded-[2rem] text-[11px] font-black uppercase flex items-center gap-4 shadow-xl active:scale-95 transition-all">
          <FileText size={20} /> PDF EXPORT
        </button>
      </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 font-black text-slate-400 uppercase tracking-widest border-b border-slate-200 text-[10px]">
                      <tr>
                        ...
                        <th className="px-8 py-6">PERSONNEL & DUTY ROLES</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {Object.entries(assignmentsByShift).map(([sid, assigs], idx) => {
                        ...
                        const shortage = sh && assignedIds.length < sh.minStaff;
                          ...
                          <tr key={sid} className="hover:bg-slate-50/50 transition-colors align-top">
                            ...
                            <td className="px-8 py-8 border-r border-slate-100">
                              <div className="space-y-3">
                                {uniqueFlights.map(f => (
                                  <div key={f!.id} className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-600 bg-slate-100/50 px-3 py-2 rounded-xl border border-slate-100">
