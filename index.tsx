        ...
        <div className="flex items-center gap-4">
          <nav className="hidden xl:flex items-center gap-1 p-1 bg-slate-100 rounded-2xl">
            {navigationTabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase italic ${activeTab === tab.id ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>
                {tab.label}
              </button>
            ))}
          </nav>
          ...
          <div className="space-y-6 md:space-y-8">
             <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
                 {[
                   ...
                   { label: 'Efficiency', val: `${stationHealth}%`, icon: Zap, color: 'text-blue-400', bg: 'bg-slate-900' }
                 ].map((stat, i) => (
